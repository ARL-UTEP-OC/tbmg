...
void
Ipv4AddressHelper::AssignIpToithNetDevice (const NetDeviceContainer &c,Ipv4InterfaceContainer &ipc, int index, Ipv4Address ip)
{
  NS_LOG_FUNCTION_NOARGS ();
  //Ipv4InterfaceContainer retval;
  
      Ptr<NetDevice> device = c.Get (index);
      ...
      Ipv4InterfaceAddress ipv4Addr = Ipv4InterfaceAddress (ip, m_mask);
      ipv4->AddAddress (interface, ipv4Addr);
      ipv4->SetMetric (interface, 1);
      ipv4->SetUp (interface);
      ipc.Add (ipv4, interface);
