...
main (int argc, char *argv[])
{
  ...
  NS_LOG_INFO ("Hardware In The Loop Example");

  std::string deviceName ("eth0");
  std::string remote (""); // example.com
  ...
  NetDeviceContainer devices = emu.Install (n0);
...
//Create the model for the first node:
    Helper HelperHelper0 = Helper(localIp);//might need to change
    HelperHelper0.SetAttribute("Remote", Ipv4AddressValue(remoteIp));
    HelperHelper0.SetAttribute("Verbose", BooleanValue(true));
    HelperHelper0.SetAttribute("Interval", TimeValue(Seconds(10)));
   HelperHelper0.SetAttribute("Instantiator", BooleanValue(true));
    HelperHelper0.SetAttribute("ID", UintegerValue(1));
    ApplicationContainer apps = HelperHelper0.Install(n0);
